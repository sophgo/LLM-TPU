...
public:
    ...
    ChatPipe(int devid, const std::string& model_path, const std::string& vocab_path, const std::string& system_prompt);
    // 聊天主循环
    void chat();
private:
    Qwen2_5VL model;
    ...
    // 获取窗口索引和累积窗口序列长度
    std::pair<std::vector<int>, std::vector<int>> get_window_index(const std::vector<std::vector<int>>& grid_thw);

    // 获取注意力掩码
    std::vector<float> get_attn_mask(int seq_length, const std::vector<int>& cu_seqlens);

    // 计算旋转位置编码
    std::vector<std::vector<int>> rot_pos(const std::vector<std::vector<int>>& grid_thw);

    // 构建提示
    std::string build_prompt(std::string input_str);

    // 获取媒体类型
    std::string get_media_type(const std::string& file_path);

    // 重新整理hidden_states
    std::vector<float> reorder_hidden_states(const std::vector<float>& hidden_states, int seq_len, const std::vector<int>& window_index);   

    // 获取rope索引
    std::vector<std::vector<std::vector<int>>> get_rope_index(const std::vector<std::vector<int>>& input_ids, const std::vector<std::vector<int>>& grid_thw, int pad_id);
...
// ChatPipe 类构造函数
ChatPipe::ChatPipe(int devid, const std::string& model_path, const std::string& vocab_path, const std::string& system_prompt) {
    ...
    config.MAX_PIXELS = 16384 * 28 * 28;
    // config.MIN_PIXELS = 4 * 28 * 28;
    maker = std::make_unique<Maker>(config);
}

// 获取窗口索引和累积窗口序列长度
std::pair<std::vector<int>, std::vector<int>> ChatPipe::get_window_index(const std::vector<std::vector<int>>& grid_thw) {
                ...
                for (int ww = 0; ww < num_windows_w; ++ww) {
                    ...
                    for (int i = 0; i < vit_merger_window_size; ++i) {
                        for (int j = 0; j < vit_merger_window_size; ++j) {
                            ...
                            if (val != -100) window.push_back(val);
                        }
                    }
                    seqlens.push_back(window.size());
                    for (int v : window) index_new.push_back(v + window_index_id);
                }
            }
        }
...
// 获取注意力掩码
std::vector<float> ChatPipe::get_attn_mask(int seq_length, const std::vector<int>& cu_seqlens) {
    std::vector<float> attention_mask(seq_length * seq_length, -10000.0f);
    for (size_t i = 1; i < cu_seqlens.size(); ++i) {
        int start = cu_seqlens[i - 1];
        int end = cu_seqlens[i];
        // 添加边界检查，避免越界访问
        if (start >= seq_length) continue;
...
// 计算旋转位置编码
std::vector<std::vector<int>> ChatPipe::rot_pos(const std::vector<std::vector<int>>& grid_thw) {
...
// 将二维向量在指定维度上拼接
std::vector<std::vector<int>> cat(const std::vector<std::vector<std::vector<int>>>& vecs, int dim) {
    if (dim == 1) {
...
std::vector<std::vector<std::vector<int>>> ChatPipe::get_rope_index(
    ...
    std::vector<std::vector<int>> attention_mask(batch_size, std::vector<int>(seq_length, 1));
    std::vector<std::vector<std::vector<int>>> position_ids(3,
        std::vector<std::vector<int>>(batch_size, std::vector<int>(seq_length, 1)));
    ...
    for (size_t i = 0; i < batch_size; ++i) {
        ...
        // 计算图像数量
        int image_nums = 0;
        std::vector<size_t> vision_start_indices = argwhere(valid_input_ids, ID_VISION_START);
        for (size_t idx : vision_start_indices) {
            if (idx + 1 < valid_input_ids.size() && valid_input_ids[idx + 1] == pad_id) {
                ++image_nums;
            }
        }
...
void ChatPipe::chat() {
    ...
    while (true) {
        std::string input_str = "what is this?";
        // std::cout << "\nQuestion: ";
        // std::getline(std::cin, input_str);
        if (input_str == "exit" || input_str == "q" || input_str == "quit") break;

        std::string media_path = "./test.jpg";
        // std::cout << "\nImage or Video Path: ";
        ...
        std::replace(raw_tokens.begin(), raw_tokens.end(), VISION_PAD_TOKEN, IMAGE_PAD_TOKEN);
        std::vector<float> pixel_values = process_image(media_path, &config);
        ...
        std::vector<std::vector<std::vector<int>>> position_ids = get_rope_index({tokens}, {config.grid_thw}, ID_IMAGE_PAD);
        ...
        for (const auto& two_dim_tensor : position_ids) {
            for (const auto& one_dim_tensor : two_dim_tensor) {
                position_ids_1d.insert(position_ids_1d.end(), one_dim_tensor.begin(), one_dim_tensor.end());
            }
        }
        ...
        std::string text;
        while (token != ID_IM_END && token != ID_END && model.token_length < model.SEQLEN) {
...
// 获取媒体类型
std::string ChatPipe::get_media_type(const std::string& file_path) {
    std::string ext = file_path.substr(file_path.find_last_of('.') + 1);
    std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
    if (ext == "jpg" || ext == "jpeg" || ext == "png" || ext == "bmp" || ext == "webp")
        return "image";
    if (ext == "mp4" || ext == "avi" || ext == "mov" || ext == "mkv" || ext == "flv" || ext == "wmv")
        return "video";
    return "unknown";
}
...
int ChatPipe::find_token_offset(const std::vector<int>& input_ids, int pad_id) {
    for (size_t i = 0; i < input_ids.size(); ++i)
        if (input_ids[i] == pad_id) return static_cast<int>(i);
    return 0;
}

std::vector<float> ChatPipe::reorder_hidden_states(const std::vector<float>& hidden_states, int seq_len, const std::vector<int>& window_index) {
    int w = seq_len;
    ...
    std::vector<std::vector<std::vector<float>>> reshaped_1(first_dim, std::vector<std::vector<float>>(spatial_merge_unit, std::vector<float>(h)));
    ...
    // 执行索引操作
    std::vector<std::vector<std::vector<float>>> indexed(window_index.size(), std::vector<std::vector<float>>(spatial_merge_unit, std::vector<float>(h)));
...
void ChatPipe::vit_process_image(std::vector<float> pixel_values, int vit_offset) {
    ...
    // std::cout << "pos_ids_vec size: " << pos_ids_vec.size() << "  " << pos_ids_vec[0].size() << std::endl;
    ...
    std::vector<float> processed_hidden_states = reorder_hidden_states(pixel_values, seq_len, window_index);
    ...
    for (size_t i = 0; i < window_index.size(); ++i) reverse_indices[window_index[i]] = i;

    model.forward_vit(processed_hidden_states, int_processed_position_ids, full_attn_mask, window_attn_mask, config.grid_thw, reverse_indices, vit_offset);
}
...
void ChatPipe::update_history(const std::string& input_str, const std::string& result) {
...
int main(int argc, char* argv[]) {
    // if (argc < 3) {
    //     std::cout << "Usage: qwen2_5vl <bmodel_path> <vocab_path> [device_id]" << std::endl;
    //     return 1;
    // }
    // std::string model_path = argv[1];
    // std::string vocab_path = argv[2];
    std::string model_path = "/data/ruoning/code/LLM-TPU/models/Qwen2_5_VL/cpp_demo_old/model/qwen2.5-vl-3b_w4bf16_seq2048.bmodel";
    std::string vocab_path = "./config/";
    int devid = 1;
    if (argc > 3) devid = std::stoi(argv[3]);

    std::string system_prompt = "You are a helpful assistant.";

    ChatPipe pipeline(devid, model_path, vocab_path, system_prompt);
    pipeline.chat();
    return 0;
}