...
class Qwen3_VL():
    ...
    def __init__(self, args):
        ...
        self.max_posid = [0] * self.model.BATCH
    ...
    def forward_prefill(self, position_ids, batch_idx):
        if self.model.history_length[batch_idx] == 0 or not self.support_history:
            self.history_max_posid[batch_idx] = 0
            return self.model.forward_first(position_ids, batch_idx)
        self.max_posid[batch_idx] += self.history_max_posid[batch_idx]
        position_ids = position_ids + self.history_max_posid[batch_idx]
        return self.model.forward_first(position_ids, batch_idx)
...
=================================================================""")
        ...
        while True:
            first_duration = 0
            token_batch = []
            for batch_idx in range(self.model.BATCH):
                ...
                if self.support_history:
                    if (token_len + self.model.history_length[batch_idx] > self.model.SEQLEN - 128) or \
                    (self.model.history_length[batch_idx] > self.model.PREFILL_KV_LENGTH):
                ...
                self.model.forward_embed(inputs.input_ids.numpy(), batch_idx)
                if media_type == "image":
                    vit_start = time.time()
                    ...
                    position_ids = self.get_rope_index(inputs.input_ids, inputs.image_grid_thw,
                                                    self.ID_IMAGE_PAD)
                    self.max_posid[batch_idx] = int(position_ids.max())
                    token = self.forward_prefill(position_ids.numpy(), batch_idx)
                elif media_type == "video":
                    vit_start = time.time()
                    ...
                    self.max_posid[batch_idx] = token_len - 1
                    token = self.forward_prefill(np.array(position_ids, dtype=np.int32), batch_idx)
            ...
            # Following tokens
            full_word_tokens = []
            full_word_tokens_batch = [[] for _ in range(self.model.BATCH)]
            text_batch = ["" for _ in range(self.model.BATCH)]
            finish_flags = [False for _ in range(self.model.BATCH)]
            while not all(finish_flags):
                # Print batch 0
                if not finish_flags[0]:
                    full_word_tokens.append(token_batch[0])
                ...
                for batch_idx in range(self.model.BATCH):
                    if token_batch[batch_idx] in [self.ID_END, self.ID_IM_END] or \
                    self.model.history_length[batch_idx] > self.model.SEQLEN - 5:
                        finish_flags[batch_idx] = True
                    if finish_flags[batch_idx]:
                        continue
                    full_word_tokens_batch[batch_idx].append(token_batch[batch_idx])
                    self.max_posid[batch_idx] += 1
                position_ids = np.array([self.max_posid, self.max_posid, self.max_posid],
                                        dtype=np.int32).transpose(1, 0)
                token_batch = self.model.forward_next(position_ids)   # multi-batch
                tok_num += self.model.BATCH
            for batch_idx in range(self.model.BATCH):
                text_batch[batch_idx] = self.tokenizer.decode(
                    full_word_tokens_batch[batch_idx], skip_special_tokens=True)
                if batch_idx > 0:
                    print(f"\n\nAnswer-{batch_idx}: \n{text_batch[batch_idx]}")
                self.history_max_posid[batch_idx] = self.max_posid[batch_idx] + 2
            next_end = time.time()
