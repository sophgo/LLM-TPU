...
std::vector<float> convertIntToFloat(const std::vector<int> &position_ids) {
  ...
  floatPositionIds.reserve(position_ids.size());
...
std::vector<int> convertFloatToInt(const std::vector<float> &position_ids) {
  ...
  intPositionIds.reserve(position_ids.size());
  for (int val : position_ids) {
...
ChatPipe::get_rope_index(const std::vector<std::vector<int>> &input_ids,
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    const std::vector<int> &tokens = input_ids[i];

    // Count media segments: a vision_start directly followed by the pad token
    int image_nums = 0;
    for (size_t idx = 0; idx + 1 < tokens.size(); ++idx) {
      if (tokens[idx] == ID_VISION_START && tokens[idx + 1] == pad_id) {
        ++image_nums;
      }
    }

    // Build the three rows directly with a running max instead of assembling
    // per-segment vectors and concatenating them afterwards.
    ...
    for (int img_idx = 0; img_idx < image_nums; ++img_idx) {
      size_t ed = tokens.size();
      auto it = std::find(tokens.begin() + st, tokens.end(), pad_id);
      if (it != tokens.end()) {
        ed = it - tokens.begin();
      }
      ...
      auto &row_h = rows[1];
      auto &row_w = rows[2];
      for (int j = 0; j < llm_grid_t; j++) {
        row_t.insert(row_t.end(), frame,
                     offset + j * second_per_grid_t * tokens_per_second);
      }
    ...
    for (int k = 0; k < 3; ++k) {
      position_ids[k][i] = std::move(rows[k]);
    }
  }
  return position_ids;
}
...
// Flatten [3][batch][seq_len] position ids to 1D; also yields max of plane 0
static std::vector<int> flatten_position_ids(
    const std::vector<std::vector<std::vector<int>>> &position_ids,
    int &max_posid) {
  std::vector<int> out;
  out.reserve(position_ids[0][0].size() * 3);
  for (const auto &plane : position_ids) {
    for (const auto &row : plane) {
      out.insert(out.end(), row.begin(), row.end());
    }
  }
  for (const auto &row : position_ids[0]) {
    for (int val : row) {
      if (val > max_posid) {
        max_posid = val;
      }
