...
class VisionTransformer(torch.nn.Module):
    ...
    def forward(self, hidden_states, image_grid_thw, pixel_length):
        breakpoint()
        hidden_states = ViT.patch_embed(hidden_states[:pixel_length,:])
...
def convert_vision_transformer():
    # make input
    x = torch.randn(600, 1176).to(dtype=torch.float32, device=device)
    thw = torch.randn(3).to(dtype=torch.int32, device=device)
    thw = torch.tensor([[1, 20, 30]])
...
def test_net_with_mask():
    ...
    image_grid_thw = inputs.video_grid_thw
    ...
    breakpoint()
    image_embeds = vit_infer(pixel_values, image_grid_thw, pixel_values.shape[-2])  # [150, 1536]
    ...
    image_mask = (input_ids_prefill == config.image_token_id).unsqueeze(-1).expand_as(inputs_embeds)
    ...
    position_ids, rope_deltas = Qwen2VLForConditionalGeneration(config).get_rope_index(
        input_ids_prefill, None, image_grid_thw, attention_mask_prefill
    )
