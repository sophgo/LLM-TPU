...
void Qwen::init(const std::vector<int> &devices, std::string model_path) {
  ...
  for (auto d : devices) {
    ...
    if (BM_SUCCESS != status) {
      status_code = -1;
      return; // -1 : can not to create bmrt
    }
    handles.push_back(h);
  }
  bm_handle = handles[0];
  ...
  if (!ret) {
    ...
    status_code = -2;
    return; // -2 : can not to load bmodel
  }
  printf("Done!\n");
...
void Qwen::net_launch(const bm_net_info_t *net, int stage_idx) {
  ...
  // assert(ret);
  if(!ret) {
    status_code = -3; // -3 : launch_tensor_ex failed
    std::cout << "bmrt_launch_tensor_ex failed" << std::endl;
  } else {
...
PYBIND11_MODULE(chat_without_assert, m) {
