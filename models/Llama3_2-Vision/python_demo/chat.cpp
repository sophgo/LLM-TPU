...
void Llama3_2::head_launch(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
  std::vector<bm_tensor_t> in_tensors(net->input_num);
  std::vector<bm_tensor_t> out_tensors(net->output_num);
...
int Llama3_2::greedy_search(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
...
int Llama3_2::penalty_sample(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
    ...
    if (std::find(cross_attn_layers.begin(), cross_attn_layers.end(), idx) !=
      ...
      if (io_alone) {
        if (idx == cross_attn_layers[0]) {
          ...
          d2d(in1_mem, net_blocks_cache[cross_attn_layers[0]]->stages[0].input_mems[1]);
        }
      } else {
        if (idx == cross_attn_layers[0]) {
          bm_memcpy_s2d(bm_handle, in1_mem, (void *)cross_attention_mask.data());
