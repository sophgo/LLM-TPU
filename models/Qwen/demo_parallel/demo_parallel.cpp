...
private:
  ...
  // void tokenizer_encode(const std::string &input_str, std::vector<int>
  // &tokens);
...
int QwenChat::forward_first(std::vector<int> &tokens) {
  ...
  for (int i = 0; i < device_num; ++i) {
    ...
    if (is_dynamic) {
      ...
      bm_set_device_mem(
          &inputs_block[0 + i * 3].device_mem, h_bytes * token_length,
      ...
      bm_set_device_mem(&inputs_block[2 + i * 3].device_mem,
                        mask_bytes * token_length * token_length,
