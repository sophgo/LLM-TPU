...
public:
  ...
  void forward_vit(ArrayFloat const &pixel_values, ArrayInt const &position_ids,
                   ...
                   ArrayFloat const &pos_embed, int vit_offset);
...
void GLM4V::net_launch(const bm_net_info_t *net, int stage_idx) {
...
void GLM4V::init(int dev_id, std::string model_path) {
...
void GLM4V::forward_embed(ArrayInt const &tokens) {
...
void GLM4V::forward_vit(ArrayFloat const &pixel_values,
  ...
  assert(pos_embed.size() == (hw * VIT_HIDDEN_SIZE));
  ...
  auto p_pos_embed = pos_embed.request();
  ...
  bm_memcpy_s2d_partial(bm_handle, vit_in3_mem, (void *)p_pos_embed.ptr,
                        pos_embed.size() * sizeof(float));
...
int GLM4V::forward_first(ArrayInt const &position_ids) {
...
int GLM4V::forward_next(ArrayInt const &position_ids) {
...
PYBIND11_MODULE(chat, m) {
  pybind11::class_<GLM4V>(m, "GLM4V")
      .def(pybind11::init<>())
      .def("init", &GLM4V::init)
      .def("forward_embed", &GLM4V::forward_embed)
      .def("forward_vit", &GLM4V::forward_vit)
      .def("forward_first", &GLM4V::forward_first)
      .def("forward_next", &GLM4V::forward_next)
      .def("deinit", &GLM4V::deinit)
      .def_readonly("SEQLEN", &GLM4V::SEQLEN) // read SEQLEN in pipeline.py
      .def_readonly("MAX_PIXELS", &GLM4V::MAX_PIXELS)
      .def_readonly("MAX_PATCHES", &GLM4V::MAX_PATCHES)
      .def_readwrite("token_length", &GLM4V::token_length);
}
