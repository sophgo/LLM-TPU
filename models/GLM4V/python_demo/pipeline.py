...
# copied from modeling_glm4v.py and modify it to only return the grid
class Glm4vVisionEmbeddings(nn.Module):
    def __init__(self, config_path):
        super().__init__()
        self.config = AutoConfig.from_pretrained(config_path, trust_remote_code=True).vision_config
        ...
        weight_path = os.path.join(config_path, "pos_embed_weight.npy")
        pos_embed = np.load(weight_path)
        self.pos_embed_2d = torch.tensor(pos_embed)
    ...
    def forward(self, lengths, image_shapes, h_coords, w_coords) -> torch.Tensor:
        ...
        interpolated_embed_fp32 = F.grid_sample(
            self.pos_embed_2d, grid, mode="bicubic", align_corners=False, padding_mode="border"
        )

        # Reshape and convert back to original dtype
        adapted_pos_embed_fp32 = interpolated_embed_fp32.squeeze(0).squeeze(-1).permute(1, 0)

        return adapted_pos_embed_fp32
...
class GLM4V():
    ...
    def __init__(self, args):
        ...
        self.ID_THINK_START = self.tokenizer.convert_tokens_to_ids('<think>')
        self.ID_THINK_END = self.tokenizer.convert_tokens_to_ids('</think>')
        self.ID_ANS_START = self.tokenizer.convert_tokens_to_ids('<answer>')
        self.ID_ANS_END = self.tokenizer.convert_tokens_to_ids('</answer>')
        self.ID_END = self.tokenizer.convert_tokens_to_ids('<|endoftext|>')
        self.ID_IMAGE_PAD = self.tokenizer.convert_tokens_to_ids('<|image|>')
        self.ID_VIDEO_PAD = self.tokenizer.convert_tokens_to_ids('<|video|>')
        self.ID_IMAGE_START = self.tokenizer.convert_tokens_to_ids('<|begin_of_image|>')
        self.ID_IMAGE_END = self.tokenizer.convert_tokens_to_ids('<|end_of_image|>')
        self.ID_VIDEO_START = self.tokenizer.convert_tokens_to_ids('<|begin_of_video|>')
        self.ID_VIDEO_END = self.tokenizer.convert_tokens_to_ids('<|end_of_video|>')
        self.spatial_merge_size = 2
        self.spatial_merge_unit = self.spatial_merge_size**2
        self.tokens_per_second = 2

        self.vision_embed = Glm4vVisionEmbeddings(args.config_path)
    ...
    def process(self, messages):
        inputs = self.processor.apply_chat_template(messages,
                                                  ...
                                                  padding=True,
                                                  size={'shortest_edge': 64*28*28, 'longest_edge': self.model.MAX_PIXELS})
        return inputs
    ...
    def vit_process_image(self, inputs, vit_offset):
        ...
        pos_embed = self.vision_embed([[seq_len]], grid_thw, position_ids[:, 0], position_ids[:, 1])
        self.model.forward_vit(hidden_states.numpy(), position_ids.numpy(), full_mask.numpy(),
                               grid_thw.numpy(), pos_embed.numpy(), vit_offset)
    ...
    def get_rope_index(self, input_ids: torch.LongTensor, grid_thw: torch.LongTensor,
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            image_nums = 0
            vision_start_indices = torch.argwhere(input_ids == self.ID_IMAGE_START).squeeze(1)
...
=================================================================""")
        ...
        while True:
            ...
            if media_path == "":
                ...
                #     # fix: video type still has some errors
            ...
            while token not in [self.ID_IMAGE_END, self.ID_END]:
                ...
                if "�" not in word:
                    ...
                    text += word
                    if token in [self.ID_ANS_START]:
                        print("\n")
                    print(word, flush=True, end="")
                    full_word_tokens = []
                if token in [self.ID_ANS_END]:
                    break
                max_posid += 1
