...
public:
  ...
  void allocate_history_buffer(int seq_len);
...
private:
  ...
  // Hidden states are accumulated directly in system memory instead of a
  // device-side dev_buffer, since they are always consumed on the host.
  std::vector<uint16_t> sys_buffer;
};