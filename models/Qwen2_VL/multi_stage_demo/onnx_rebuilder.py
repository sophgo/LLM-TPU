...
class OnnxRebuilder:
    ...
    def export_embed(self):
        ...
        if not self.embedding_disk:
            ...
            if os.path.exists(embedding_file):
                ...
                return
            input_ids = torch.tensor([range(self.input_seq_len)], dtype=torch.int32)
    ...
    def export_block(self):
        hidden_states = torch.randn((1, self.input_seq_len, self
                                     .hidden_size), dtype=torch.float)
        position_ids = torch.tensor([range(self.input_seq_len)], dtype=torch.long)
        attention_mask = torch.randn((1, 1, self.input_seq_len, self.input_seq_len), dtype=torch.float)
