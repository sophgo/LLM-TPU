...
void Qwen3_ASR::net_launch(const bm_net_info_t *net, int stage_idx) {
...
void Qwen3_ASR::net_launch_block_dyn(const bm_net_info_t *net, int real_len) {
  std::vector<bm_tensor_t> in_tensors(net->input_num);
  std::vector<bm_tensor_t> out_tensors(net->output_num);
...
void Qwen3_ASR::d2d(bm_device_mem_t &dst, bm_device_mem_t &src) {
  bm_memcpy_d2d_byte(bm_handle, dst, 0, src, 0, bm_mem_get_device_size(src));
}

void Qwen3_ASR::clear_history() {
  if (!support_history) {
    return;
  }
...
void Qwen3_ASR::init_by_names() {
  ...
  // 4 nets are embed, lm_head, embedding_cache, audio
  ...
  AUDIO_LENGTH = net_audio->stages[0].input_shapes[0].dims[3];
  KV_BYTES =
...
void Qwen3_ASR::init(int dev_id, std::string model_path) {
...
int Qwen3_ASR::greedy_search(bm_device_mem_t &logits_mem) {
...
void Qwen3_ASR::forward_embed(ArrayInt const &tokens) {
...
int Qwen3_ASR::forward_first(ArrayInt const &position_ids) {
  // if (support_history) {
...
int Qwen3_ASR::forward_next(ArrayInt const &position_ids) {
...
PYBIND11_MODULE(chat, m) {
  pybind11::class_<Qwen3_ASR>(m, "Qwen3_ASR")
      .def(pybind11::init<>())
      .def("init", &Qwen3_ASR::init)
      .def("forward_embed", &Qwen3_ASR::forward_embed)
      .def("forward_audio", &Qwen3_ASR::forward_audio)
      .def("forward_first", &Qwen3_ASR::forward_first)
      .def("forward_next", &Qwen3_ASR::forward_next)
      .def("clear_history", &Qwen3_ASR::clear_history)
      .def("deinit", &Qwen3_ASR::deinit)
      .def_readonly("SEQLEN", &Qwen3_ASR::SEQLEN) // read SEQLEN in pipeline.py
      .def_readonly("AUDIO_LENGTH", &Qwen3_ASR::AUDIO_LENGTH)
      .def_readonly("MAX_INPUT_LENGTH", &Qwen3_ASR::MAX_INPUT_LENGTH)
      .def_readonly("PREFILL_KV_LENGTH", &Qwen3_ASR::PREFILL_KV_LENGTH)
      .def_readonly("support_history", &Qwen3_ASR::support_history)
      .def_readonly("history_length", &Qwen3_ASR::history_length);
}
