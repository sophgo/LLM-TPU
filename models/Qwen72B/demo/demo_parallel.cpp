...
void QwenChat::load_model(const std::string &model_path) {
...
int main(int argc, char **argv) {
  ...
  processArguments(argc, argv, qwen_model, tokenizer_path, devices);
  if (qwen_model.empty()) {
    Usage();
    exit(EXIT_FAILURE);
  }
  ...
  qwen.init(devices, qwen_model, tokenizer_path);
  printf("==========================\n");
  qwen.chat();
  qwen.deinit();
  return 0;
