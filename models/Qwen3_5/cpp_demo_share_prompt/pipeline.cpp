...
void Usage() {
  printf(
      ...
      "                      based on\n"
      "  -t, --prompt_file : Path to a text file whose contents are used as\n"
      "                      the shared prompt. If --prompt is also set, the\n"
      "                      file contents come first, followed by the\n"
      "                      --prompt value (combined with a newline)\n"
      "  -i, --media_path  : Image/video path(s) in the shared prompt\n"
...
void processArguments(int argc, char *argv[], std::string &model_path,
  ...
  int optionIndex = 0;
  int option;
  while ((option = getopt_long(argc, argv, "m:c:d:r:f:p:t:i:w:sh", longOptions,
                               &optionIndex)) != -1) {
    switch (option) {
    case 'm':
      model_path = optarg;
...
int main(int argc, char *argv[]) {
  ...
  // Shared-prompt mode: --prompt/--prompt_file/--media_path are only used to generate
  // the kv cache and states, then enter interactive chat where each question is based on the shared prompt
  if (!has_prompt && media_path.empty()) {
    std::cerr << "Error: --prompt/--prompt_file and/or --media_path is "
                 "required as the shared prompt"
              << std::endl;
    Usage();
    exit(EXIT_FAILURE);
  }
