...
public:
  ...
  void deinit();
  void forward_embed(ArrayInt const &tokens, int batch_idx);
  ...
  int forward_first(ArrayInt const &position_ids, int batch_idx);
  std::vector<int> forward_next(ArrayInt const &position_ids);
  void clear_history();
...
private:
  ...
  void net_launch_decode(int block_idx, std::vector<int> kv_offset,
...
private:
  ...
  std::vector<bm_device_mem_t> lm_out_mems;
};
...
void Qwen3_VL::init_by_names() {
  ...
  BATCH = net_blocks_cache[0]->stages[0].input_shapes[0].dims[0];
  SEQLEN = net_blocks_cache[0]->stages[0].input_shapes[3].dims[1];
  token_lengths.resize(BATCH);
  history_length.resize(BATCH);
...
void Qwen3_VL::init(int dev_id, std::string model_path) {
  ...
  lm_out_mems.resize(BATCH);
  int lm_out_size = bm_mem_get_device_size(net_lm->stages[0].output_mems[0]);
  for (int i = 0; i < BATCH; i++) {
    bm_malloc_device_byte(bm_handle, &lm_out_mems[i], lm_out_size);
    empty(bm_handle, lm_out_mems[i]);
  }
  auto buffer_size =
...
void Qwen3_VL::forward_embed(ArrayInt const &tokens, int batch_idx) {
  ...
  token_lengths[batch_idx] = tokens.size();
...
int Qwen3_VL::forward_first(ArrayInt const &position_ids, int batch_idx) {
  if (support_history) {
  ...
  for (int idx = 0; idx < NUM_LAYERS; idx++) {
    ...
    int offset = batch_idx * KV_BYTES / BATCH * SEQLEN;
  ...
  net_launch(net_lm);
  d2d(lm_out_mems[batch_idx], lm_out_mem);
  int token = 0;
  if (lmhead_with_topk) {
  ...
  token_lengths[batch_idx]++;
  history_length[batch_idx] = token_lengths[batch_idx];
  return token;
}
...
std::vector<int> Qwen3_VL::forward_next(ArrayInt const &position_ids) {
  ...
  std::string embedding_cache_name;
  for (int batch_idx = 0; batch_idx < BATCH; batch_idx++) {
    for (int i = history_length[batch_idx]; i < SEQLEN - 1; i++) {
      attention_mask[i + batch_idx * SEQLEN] = mask_value;
    }
    // embedding
    auto in_mem = net_embed_cache->stages[0].input_mems[0];
    auto out_mem = net_embed_cache->stages[0].output_mems[0];
    if (lmhead_with_topk) {
      d2d(in_mem, lm_out_mems[batch_idx]);
    } else {
      // TODO
  ...
  for (int idx = 0; idx < NUM_LAYERS; idx++) {
    ...
    hidden_states = net_blocks_cache[idx]->stages[0].output_mems[0];
  }

  std::vector<int> token(BATCH, 0);
  for (int batch_idx = 0; batch_idx < BATCH; batch_idx++) {
    // forward lmhead
    bm_memcpy_d2d_byte(bm_handle, net_lm->stages[0].input_mems[0], 0,
...
PYBIND11_MODULE(chat, m) {
  pybind11::class_<Qwen3_VL>(m, "Qwen3_VL")
      ...
      .def_readonly("BATCH", &Qwen3_VL::BATCH)
