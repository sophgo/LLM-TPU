...
void MiniCPM3::head_launch(const bm_net_info_t *net,
...
int MiniCPM3::greedy_search(const bm_net_info_t *net,
                            bm_device_mem_t &logits_mem) {
...
int MiniCPM3::penalty_sample(const bm_net_info_t *net,
                             bm_device_mem_t &logits_mem) {
...
int MiniCPM3::forward_next() {
  ...
  int v_bytes =
      bm_mem_get_device_size(net_blocks_cache[0]->stages[0].output_mems[2]);
  for (int idx = 0; idx < NUM_LAYERS; idx++) {
    ...
    out_mem = out0_mem;
    bm_memcpy_d2d_byte(bm_handle, past_key[idx], (token_length - 1) * k_bytes,
                       out1_mem, 0, k_bytes);
    bm_memcpy_d2d_byte(bm_handle, past_value[idx], (token_length - 1) * v_bytes,
                       out2_mem, 0, v_bytes);
  }
