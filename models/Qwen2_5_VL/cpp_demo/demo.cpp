...
static int resized_height = 280;
static int resized_width = 420;
...
class Qwen2VL {
public:
  void init(std::string model_path, std::string processor_path,
...
void Qwen2VL::init(std::string model_path, std::string processor_path,
...
void processArguments(int argc, char *argv[], std::string &model_path,
                      std::string &processor_path, std::string &image_path,
  ...
  while ((option = getopt_long(argc, argv, "m:c:i:h:w:d:e?", longOptions,
    ...
    case 'i':
      ...
      break;
    case 'h':
      resized_height = std::atoi(optarg);
      break;
    case 'w':
      resized_width = std::atoi(optarg);
      break;
    case 'd':
      devices = {atoi(optarg)};
      break;
...
int main(int argc, char **argv) {
  ...
  model.init(model_path, processor_path, system_prompt, enable_history,
             devices);
