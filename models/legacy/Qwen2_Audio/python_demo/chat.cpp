...
public:
  ...
  forward(ArrayFloat const &input_embeds, ArrayInt const &position_ids,
          ArrayFloat const &input_attention_mask, const int idx);
  std::tuple<std::vector<float>, std::vector<float>, std::vector<float>>
  forward_cache_next(ArrayFloat const &inputs_embeds,
                     ArrayFloat const &position_ids,
                     ArrayFloat const &attention_mask,
                     ArrayFloat const &past_key_array,
                     ArrayFloat const &past_value_array, const int idx);
  ;
...
void Qwen2Audio::init_by_names() {
  ...
  for (int i = 0; i < NUM_LAYERS; i++) {
    ...
    // std::cout << "block name: " << block_name << " " << cache_name <<
    // std::endl;
...
void Qwen2Audio::init(int dev_id, std::string model_path) {
  ...
  AUDIO_DIMS = net_audio->stages[0]
                   .input_shapes[0]
                   .dims[1]; // [1, 128, 3000],[1,1,1500, 1500]
...
ArrayFloat Qwen2Audio::forward_embed(ArrayInt const &input_ids) {
  ...
  return py::array_t<float>(host_output_buffer.size(),
...
ArrayFloat Qwen2Audio::forward_embed_cache(ArrayInt const &input_ids) {
  ...
  const size_t total_elements =
      net_embed_cache->stages[0].output_shapes[0].dims[0] *
...
ArrayFloat Qwen2Audio::forward_audio(ArrayFloat const &input_features,
                                     ArrayFloat const &attention_maks) {
...
ArrayFloat Qwen2Audio::forward_project(ArrayFloat const &audio_features) {
  ...
  const size_t total_elements =
      net_project->stages[0].output_shapes[0].dims[0] *
...
std::tuple<std::vector<float>, std::vector<float>, std::vector<float>>
Qwen2Audio::forward(ArrayFloat const &input_embeds,
  ...
  const size_t kv_total_elements =
      net_blocks[idx]->stages[0].output_shapes[2].dims[0] *
  ...
  const size_t inputs_embeds_total_elements =
      net_blocks[idx]->stages[0].output_shapes[0].dims[0] *
  ...
  std::vector<float> input_embeds_host_input_buffer(
      inputs_embeds_total_elements);
  bm_memcpy_d2s(bm_handle, (void *)input_embeds_host_input_buffer.data(),
                out0_mem);

  return std::make_tuple(input_embeds_host_input_buffer, k_host_output_buffer,
                         v_host_output_buffer);
}
...
Qwen2Audio::forward_cache_next(ArrayFloat const &inputs_embeds,
  ...
  const size_t v_total_elements =
      net_blocks_cache[idx]->stages[0].output_shapes[2].dims[0] *
  ...
  const size_t embedding_total_elements =
      net_blocks_cache[idx]->stages[0].output_shapes[0].dims[0] *
  ...
  bm_memcpy_d2s(bm_handle, (void *)input_embeds_host_input_buffer.data(),
                out0_mem);
  // return py::array_t<float>(input_embeds_host_input_buffer.size(),
  // input_embeds_host_input_buffer.data(), py::cast(new
  // std::vector<float>(std::move(input_embeds_host_input_buffer))));
  return std::make_tuple(input_embeds_host_input_buffer, k_host_output_buffer,
                         v_host_output_buffer);
}
