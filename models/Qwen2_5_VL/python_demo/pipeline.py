    ...
    def run_once(self, input_str, media_path=""):
        ...
        if token_len > self.model.MAX_INPUT_LENGTH:
            if media_type in ["image", "video"]:
                grid_info = inputs.image_grid_thw if media_type == "image" else inputs.video_grid_thw
                print("grid_thw:{}".format(grid_info))
            print(
        ...
        if self.support_history:
            if (token_len + self.model.history_length > self.model.SEQLEN - 128) or             (self.model.history_length > self.model.PREFILL_KV_LENGTH):
        ...
        if media_type == "image":
            ...
            print(f"VIT({inputs.video_grid_thw.tolist()[0]}): {vit_end - vit_start:.3f} s")
        return text
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--prompt', type=str, default=None,
                        help='If set, run programmatically (non-interactive): a single inference is performed using this prompt and then the program exits. Include @<path> to attach an image or video.')
    # yapf: enable
