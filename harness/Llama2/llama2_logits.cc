...
class LLama2 {
   public:
    ...
    void predict_first_token(const std::string& input_str);
    void predict_next_token();

    std::string complete(std::string_view input_str);
    std::string answer_v1(const std::string& input_str);

    int decode_logits(const std::vector<uint16_t>& logits);

    std::string decode_self() {
        int         token = decode_logits(logits);
        std::string res;
        sentencepiece.Decode(std::vector<int>{token}, &res);
        return res;
    }
    std::string decode_token(half* logits) {
        int token = decode_logits(std::vector<half>(logits, logits + 32000));
        std::string res;
        sentencepiece.Decode(std::vector<int>{token}, &res);
        return res;
    }
    ...
    void              forward_next();
    std::string       build_prompt(std::string_view input_str);
...
int LLama2::decode_logits(const std::vector<half>& logits) {
    float mx = -9999;
    int   idx = -1, n = logits.size();
    for (int i = 0; i < n; i++) {
        auto x = half_to_float(logits[i]);
        if (mx < x) mx = x, idx = i;
    }
    return idx;
}

// std::string decode(const half* logits) {
//     float mx = -9999;
//     int   idx = -1, n = 32000;
//     for (int i = 0; i < n; i++) {
//         auto x = half_to_float(logits[i]);
...
void LLama2::forward_first(std::vector<int>& tokens) {
    // make inputs
    ...
    std::vector<half> results(32000);

    // int token = 0;
    bm_memcpy_d2s(bm_handle, logits.data(), outputs_lm[0].device_mem);
    int token = decode_logits(logits);
    // std::cout << decode_logits(results);
    last_token = token;
    // return token;
}

void LLama2::forward_next() {
    ...
    bm_tensor_t input_token;
    bmrt_tensor_ex(
            &input_token,
            p_bmrt,
            0,
            net_embed_cache->input_dtypes[0],
            net_embed_cache->stages[0].input_shapes[0]);
    bm_memcpy_s2d(bm_handle, input_token.device_mem, &last_token);

    for (int i = 0; i < device_num; ++i) {
        inputs_embed.push_back(input_token); // token_id
        inputs_embed[i].shape = net_embed_cache->stages[0].input_shapes[0];
        inputs_embed[i].dtype = net_embed_cache->input_dtypes[0];
        input_datas.push_back((void*)(&last_token));
    }
...
std::string LLama2::build_prompt(std::string_view input_str) {
    history = std::string{sys_config} + std::string{input_str} + " [/INST] ";
    return history;
}
...
void LLama2::predict_first_token(const std::string& input_str) {
    history = input_str;
    // int tok_num = 1;
    std::vector<int> tokens;
    sentencepiece.Encode(history, &tokens);
...
void LLama2::predict_next_token() {
    // int pre_token = 0;
    forward_next();
    int token = decode_logits(logits);
    if (token == EOS) {
...
const char* Llama2_predict_first_token(LLama2* chat, const char* input_str) {
    auto prompt =
            std::string{sys_config} + std::string{input_str} + " [/INST] ";
    chat->predict_first_token(prompt);
    result = chat->decode_self();
    return result.c_str();
}

half* Llama2_predict_first_token_logits(LLama2* chat, const char* input_str) {
    // auto prompt =
    //         std::string{sys_config} + std::string{input_str} + " [/INST] ";
    auto prompt = std::string{input_str};
    chat->predict_first_token(prompt);
    return chat->logits.data();
}

const char* Llama2_predict_next_token(LLama2* chat) {
    chat->predict_next_token();
    result = chat->decode_self();
    return result.c_str();
}

half* Llama2_predict_next_token_logits(LLama2* chat) {
    chat->predict_next_token();
    return chat->logits.data();
}
