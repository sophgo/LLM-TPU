...
void InternVL3::net_launch_kv_dyn(const bm_net_info_t *net, int real_len,
                                  int kv_len) {
  std::vector<bm_tensor_t> in_tensors(net->input_num);
  std::vector<bm_tensor_t> out_tensors(net->output_num);
...
void InternVL3::forward_vit(ArrayFloat const &pixel_values, int vit_offset) {
  ...
  const float *pixel_ptr = pixel_values.data();
  size_t pixel_len = pixel_values.size();
...
int InternVL3::forward_first_with_kv() {
  ...
  for (int idx = 0; idx < NUM_LAYERS; idx++) {
    ...
    if (old_length > 0) {
      ...
      net_launch_block_dyn(net, token_length);
    }
