...
public:
  ...
  std::mt19937 sgen;
  MiniCPM3() : sgen(std::random_device()()){};
...
void MiniCPM3::net_launch(const bm_net_info_t *net, int stage_idx) {
...
void MiniCPM3::d2d(bm_device_mem_t &dst, bm_device_mem_t &src) {
  bm_memcpy_d2d_byte(bm_handle, dst, 0, src, 0, bm_mem_get_device_size(src));
}

void MiniCPM3::init(const std::vector<int> &devices, std::string model_path) {
...
void MiniCPM3::head_launch(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
  std::vector<bm_tensor_t> in_tensors(net->input_num);
  std::vector<bm_tensor_t> out_tensors(net->output_num);
...
int MiniCPM3::greedy_search(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
...
int MiniCPM3::penalty_sample(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
...
int MiniCPM3::forward_first(std::vector<int> &tokens) {
...
int MiniCPM3::forward_next() {
  ...
  // blocks
  int k_bytes = bm_mem_get_device_size(net_blocks_cache[0]->stages[0].output_mems[1]);
  int v_bytes = bm_mem_get_device_size(net_blocks_cache[0]->stages[0].output_mems[2]);
  for (int idx = 0; idx < NUM_LAYERS; idx++) {
    ...
    out_mem = out0_mem;
    bm_memcpy_d2d_byte(bm_handle, past_key[idx], (token_length - 1) * k_bytes, out1_mem, 0,
                       k_bytes);
    bm_memcpy_d2d_byte(bm_handle, past_value[idx], (token_length - 1) * v_bytes, out2_mem, 0,
                       v_bytes);
  }
...
PYBIND11_MODULE(chat, m) {
    pybind11::class_<MiniCPM3>(m, "MiniCPM3")
        .def(pybind11::init<>())
        .def("init", &MiniCPM3::init)
        .def("forward_first", &MiniCPM3::forward_first)
        .def("forward_next", &MiniCPM3::forward_next)
        .def("deinit", &MiniCPM3::deinit)
        .def_readwrite("SEQLEN", &MiniCPM3::SEQLEN)
        .def_readwrite("token_length", &MiniCPM3::token_length)
        .def_readwrite("temperature", &MiniCPM3::temperature)
        .def_readwrite("top_p", &MiniCPM3::top_p)
        .def_readwrite("repeat_penalty", &MiniCPM3::repeat_penalty)
        .def_readwrite("repeat_last_n", &MiniCPM3::repeat_last_n)
        .def_readwrite("max_new_tokens", &MiniCPM3::max_new_tokens)
        .def_readwrite("generation_mode", &MiniCPM3::generation_mode)
        .def_readwrite("prompt_mode", &MiniCPM3::prompt_mode);
}
