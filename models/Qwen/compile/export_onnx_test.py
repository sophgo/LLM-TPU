...
parser.add_argument('-g', '--generation_mode', type=str, choices=["naked", "sample", 'logits'], default="mode to generate")
...
def convert_block_cache(layer_id):
    ...
    past_k = torch.randn((1, SEQ_LENGTH, NUM_ATTENTION_HEADS, HEAD_DIM)).float().to(device)
    past_v = torch.randn((1, SEQ_LENGTH, NUM_ATTENTION_HEADS, HEAD_DIM)).float().to(device)
...
def convert_lm_head():
    if generation_mode == "naked":
        model = LmHead()
        input = torch.randn(1, HIDDEN_SIZE).float().to(device)
        module = torch.jit.trace(model.forward, input)
        torch.jit.save(module, f'{folder}/lm_head.pt')
    elif generation_mode == "sample":
        model = LmHeadSample()
        input_ids = torch.tensor([range(SEQ_LENGTH)]).to(device)
        hidden_states = torch.randn(1, HIDDEN_SIZE).float().to(device)
        top_p = torch.tensor([0.8]).float().to(device)
        temperature = torch.tensor([0.98]).float().to(device)
        penalty = torch.tensor([0.98]).float().to(device)
