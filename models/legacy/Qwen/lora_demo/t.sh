...
lora_rank_num=32 # rank of the external lora branch when lora is enabled
...
lib_path="../../Qwen2/share_cache_demo/build/libcipher.so"
device="cpu"
num_thread=16
...
./compile_multi.sh --mode int4 --name qwen-7b --share_length_list $share_length_list --addr_mode io_alone --unshare_length_list $unshare_length_list --seq_length_list $seq_length_list --generation_mode $generation_mode --dynamic $dynamic --embedding_mode $embedding_mode --lora_rank_num $lora_rank_num

model_tool --encrypt -model qwen-7b.bmodel -net block_0 -lib $lib_path -o encrypted.bmodel
