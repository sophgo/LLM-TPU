...
void Yi::load_sentencepiece(std::string tokenizer_path) {
...
void Yi::init(const std::vector<int> &devices,
                    const std::string &model_path,
                    const std::string &tokenizer_path) {
  // load tokenizer
...
std::string Yi::build_prompt(std::string query, std::vector<std::pair<std::string, std::string>> history_vector) {
...
int Yi::forward_next(int cur_token) {
...
int main(int argc, char **argv) {
  ...
  printf("Demo for Yi34B in BM1684X\n");
  std::string model_path;
  std::string tokenizer_path;
  ...
  yi.init(devices, model_path, tokenizer_path);
  printf("==========================\n");
  yi.chat();
  yi.deinit();
  return 0;
