...
void Qwen::init(const std::vector<int> &devices, std::string model_path) {
  ...
  MAX_UNPROMPT_LENGTH = net_blocks_prompt_cache[0]
                            ->stages[0]
                            .input_shapes[0]
                            .dims[1]; // real seqlen
  MAX_PROMPT_LENGTH = net_blocks_prompt_cache[0]
                          ->stages[0]
                          .input_shapes[3]
                          .dims[1]; // real seqlen
...
int Qwen::forward_prompt_first(std::vector<int> &tokens) {
  ...
  std::vector<uint16_t> attention_mask(MAX_UNPROMPT_LENGTH * SEQLEN,
                                       mask_value);
  std::copy(tokens.begin(), tokens.end(), visited_tokens.data());
  ...
  // forward blocks
  int prompt_bytes = bm_mem_get_device_size(
                         net_blocks_prompt_cache[0]->stages[0].input_mems[3]) /
                     MAX_PROMPT_LENGTH;
  ...
  for (int idx = 0; idx < NUM_LAYERS; idx++) {
    ...
    d2d(past_key[idx], net_blocks_prompt_cache[idx]->stages[0].output_mems[1],
        prompt_offset);
    d2d(past_value[idx], net_blocks_prompt_cache[idx]->stages[0].output_mems[2],
        prompt_offset);
  }
  ...
  visited_tokens[token_length] = token;
  visited_tokens.insert(visited_tokens.begin(), prompt_tokens.begin(),
                        prompt_tokens.end());
  visited_tokens.erase(visited_tokens.end() - prompt_length,
                       visited_tokens.end());
  token_length += 1;
