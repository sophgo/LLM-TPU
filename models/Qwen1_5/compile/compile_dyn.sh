#!/bin/bash
# example: ./compile_dyn.sh --name qwen1.5-1.8b
set -ex
models=
mode="int4"
folder="tmp"
...
if [ x$num_device != x1 ]; then
    ...
    out_model=$name'_'$mode'_'$num_device'dev_dynamic.bmodel'
else
    out_model=$name'_'$mode'_1dev_dynamic.bmodel'
fi

outdir=${folder}/embedding_dynamic
mkdir -p $outdir
pushd $outdir
...
outdir=${folder}/$mode"_"$num_device"dev_dynamic"/lm_head
mkdir -p $outdir
pushd $outdir
...
outdir=${folder}/$mode"_"$num_device"dev_dynamic"/block
mkdir -p $outdir
