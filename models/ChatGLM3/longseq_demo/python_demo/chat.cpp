...
//   std::vector<int> position_id(WINDOW_SEQLEN, 0);
//   std::vector<uint16_t> attention_mask(WINDOW_SEQLEN * WINDOW_SEQLEN , 0);
...
//     position_id[i] = i;
//   }
//   for (int i = 0; i <WINDOW_SEQLEN; i++) {
//     for (int j = 0; j < WINDOW_SEQLEN; j++) {
//       if (j <= i && i < token_length) {
//       } else {
//         attention_mask[i * WINDOW_SEQLEN + j] = ATTENTION_MASK;
//       }
//     }
//   }
...
//   // forward blocks
//   auto &out_mem = net_blocks[0]->stages[0].output_mems[0];
//   int bytes = embed_out_mem.size / SEQLEN;
//   int window_states_bytes = bytes * block_idx * WINDOW_SEQLEN;
//   int window_kv_bytes = out_mem.size * block_idx; 
...
//     // d2d(in0_mem, out_mem);
//     if(idx == 0)
//       bm_memcpy_d2d_byte(bm_handle, in0_mem, 0, embed_out_mem, window_states_bytes , bytes * WINDOW_SEQLEN);
//     else
//       d2d(in0_mem, out_mem);
//     if (idx == 0) {
...
//     bm_memcpy_d2d_byte(bm_handle, past_key[idx], window_kv_bytes, out1_mem, 0, bm_mem_get_device_size(out1_mem));
//     bm_memcpy_d2d_byte(bm_handle, past_value[idx], window_kv_bytes, out2_mem, 0, bm_mem_get_device_size(out2_mem));
//     if(token_length > block_idx * WINDOW_SEQLEN && token_length <= (block_idx + 1) * WINDOW_SEQLEN){
//       move2end(past_key[idx]);
//       move2end(past_value[idx]);
//       if(block_idx == NUM_LAYERS - 1)
//         bm_memcpy_d2d_byte(bm_handle, lm_in_mem, 0, out_mem,
//                         (token_length % (WINDOW_SEQLEN) - 1) * bytes, bytes);
//       }
//   }
...
// int ChatGLM::forward_first(std::vector<int> &tokens) {
//   std::vector<int> position_id(SEQLEN/BLOCK_GROUP_NUM, 0);
//   std::vector<uint16_t> attention_mask(SEQLEN * SEQLEN/(BLOCK_GROUP_NUM*BLOCK_GROUP_NUM), 0);
//   std::copy(tokens.begin(), tokens.end(), visited_tokens.data());
...
//     position_id[i] = i;
//   }
//   for (int i = 0; i < SEQLEN/BLOCK_GROUP_NUM; i++) {
//     for (int j = 0; j < SEQLEN/BLOCK_GROUP_NUM; j++) {
//       if (j <= i && i < token_length % (SEQLEN/BLOCK_GROUP_NUM) ) {
//       } else {
//         attention_mask[i * SEQLEN/BLOCK_GROUP_NUM + j] = ATTENTION_MASK;
//       }
//     }
//   }
...
//   // forward blocks
//   const u_int64_t kvcache_size = bm_mem_get_device_size(net_blocks[0]->stages[0].output_mems[1]);
//   // forward lmhead
//   int bytes = out_mem.size / SEQLEN;
//   auto &lm_in_mem = net_lm->stages[0].input_mems[0];
//   auto &lm_out_mem = net_lm->stages[0].output_mems[0];
//   for (int idx = 0; idx < NUM_LAYERS; idx++) {
//     for (int block_group_idx = 0; block_group_idx < BLOCK_GROUP_NUM; block_group_idx++){
//       if(idx == 0)
//         out_mem = net_embed->stages[0].output_mems[0];
//       // out_mem = net_embed->stages[0].output_mems[0];
//       auto &in0_mem = net_blocks[idx]->stages[0].input_mems[0];
//       auto &in1_mem = net_blocks[idx]->stages[0].input_mems[1];
//       auto &in2_mem = net_blocks[idx]->stages[0].input_mems[2];
//       // d2d(in0_mem, out_mem);
//       if(idx == 0)
//         bm_memcpy_d2d_byte(bm_handle, in0_mem, 0, out_mem, bm_mem_get_device_size(out_mem)/BLOCK_GROUP_NUM*block_group_idx, bm_mem_get_device_size(out_mem)/BLOCK_GROUP_NUM);
//       else
//         d2d(in0_mem, out_mem);
//       if (idx == 0 && block_group_idx == 0) {
//         // only first time need copy
//         bm_memcpy_s2d(bm_handle, in1_mem, (void *)position_id.data());
//         bm_memcpy_s2d(bm_handle, in2_mem, (void *)attention_mask.data());
//       }
...
//       // d2d(past_key[idx], net_blocks[idx]->stages[0].output_mems[1]);
//       // d2d(past_value[idx], net_blocks[idx]->stages[0].output_mems[2]);
//       bm_memcpy_d2d_byte(bm_handle, past_key[idx], kvcache_size * block_group_idx, net_blocks[idx]->stages[0].output_mems[1], 0, kvcache_size);
//       bm_memcpy_d2d_byte(bm_handle, past_value[idx], kvcache_size * block_group_idx, net_blocks[idx]->stages[0].output_mems[2], 0, kvcache_size);

//       if(token_length > block_group_idx * SEQLEN / BLOCK_GROUP_NUM && token_length <= (block_group_idx + 1) * SEQLEN / BLOCK_GROUP_NUM && idx == NUM_LAYERS - 1){
//         bm_memcpy_d2d_byte(bm_handle, lm_in_mem, 0, out_mem,
//                       (token_length % (SEQLEN / BLOCK_GROUP_NUM)  - 1) * bytes, bytes);
//         break;
//       }
//     }
//     move2end(past_key[idx]);
