...
public:
  ...
  ChatPipe(std::vector<int> devids, float video_ratio, float video_fps,
...
// ChatPipe 类构造函数
ChatPipe::ChatPipe(std::vector<int> devids, float video_ratio, float video_fps,
                   const std::string &model_path,
                   const std::string &config_path, bool do_sample) {
  model.init(devids, model_path, config_path, do_sample);
...
void Usage() {
  printf(
      ...
      "  -m, --model       : Set model path (directory containing bmodels)\n"
...
static std::vector<int> parseDevIds(const std::string &s) {
  std::vector<int> ids;
  std::stringstream ss(s);
  std::string item;
  while (std::getline(ss, item, ',')) {
...
void processArguments(int argc, char *argv[], std::string &model_path,
  ...
  int optionIndex = 0;
  int option;
  while ((option = getopt_long(argc, argv, "m:c:d:r:f:sp:t:i:h", longOptions,
                               &optionIndex)) != -1) {
    switch (option) {
    case 'm':
      model_path = optarg;
