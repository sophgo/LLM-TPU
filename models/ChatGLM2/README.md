...
python3 export_onnx.py --model_path your_chatglm2_model_path
```
...
若想进行INT8或INT4量化，则执行以下命令，最终生成`chatglm2-6b_int8.bmodel`或`chatglm2-6b_int4.bmodel`文件，如下命令：
...
编译生成chatglm可执行程序，将`chatglm`放到/ChatGLM2-TPU/demo目录下，同时按照下列方式指定芯片数量和bmodel路径。
运行`chatglm`，默认单芯运行`chatglm2-6b.bmodel`:
```shell
./chatglm --model chatglm2-6b.bmodel --tokenizer ../support/tokenizer.model
```

如果是要运行INT8或INT4模型，则命令如下：
```shell
./chatglm --model chatglm2-6b_int8.bmodel --tokenizer ../support/tokenizer.model # same with int4
```
