...
void Qwen3_VL::init(int dev_id, std::string model_path, std::string config_path,
                    bool do_sample_, bool in_device_, int repetition_window_) {
  ...
  if (in_device_ == false) {
    ...
    size_t model_size = infile.tellg();
    infile.seekg(0, std::ios::beg);
    printf("model_size: %lu bytes\n", model_size);
    status = bm_malloc_device_byte_u64(bm_handle, &bmodel_mem, model_size);
    assert(BM_SUCCESS == status);
    void *bmodel_ptr = nullptr;
    status = bm_mem_mmap_device_mem_u64(bm_handle, &bmodel_mem,
                                        (unsigned long long *)&bmodel_ptr);
    assert(BM_SUCCESS == status);
    infile.read((char *)bmodel_ptr, model_size);
    infile.close();
    uint64_t model_addr = bm_mem_get_device_addr_u64(bmodel_mem);
    bool ret =
        bmrt_load_bmodel_in_device(p_bmrt, bmodel_ptr, model_addr, model_size);
    assert(true == ret);
    in_device = true;
    status = bm_mem_flush_device_mem_u64(bm_handle, &bmodel_mem);
    assert(BM_SUCCESS == status);
    bm_mem_unmap_device_mem_u64(bm_handle, &bmodel_mem, model_size);
    printf("Bmodel loaded to device memory at address 0x%lx\n", model_addr);
  }
...
void Qwen3_VL::deinit() {
  ...
  if (in_device) {
    bm_free_device_u64(bm_handle, bmodel_mem);
  }
...
void Qwen3_VL::lora_load(lora_cache_ptr_t lora_cache) {
  if (lora_cache->empty()) {
    std::cerr << "Warning: No valid LoRA weights found in the directory. Skip "
                 "LoRA loading."
              << std::endl;
    return;
  }
