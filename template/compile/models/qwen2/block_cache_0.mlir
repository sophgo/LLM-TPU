...
module @block_cache_0 attributes {module.FLOPs = 598976996 : i64, module.chip = "ALL", module.platform = "ONNX", module.state = "TOP_F32", module.top_run_mode = "STATIC", module.weight_file = "block_cache_0_top_f32_all_weight.npz"} {
  func.func @main(%arg0: tensor<1x1x{hidden_size}xf32> loc(unknown), %arg1: tensor<1x1xsi32> loc(unknown), %arg2: tensor<1x1x1x{seq_next}xf32> loc(unknown), %arg3: tensor<1x{seq_length}x{num_key_value_heads}x{head_dim}xf32> loc(unknown), %arg4: tensor<1x{seq_length}x{num_key_value_heads}x{head_dim}xf32> loc(unknown)) -> (tensor<1x1x{hidden_size}xf32>, tensor<1x1x{num_key_value_heads}x{head_dim}xf32>, tensor<1x1x{num_key_value_heads}x{head_dim}xf32>) {
    ...
    %7 = "top.RMSNorm"(%1, %6) {eps = {rms_norm_eps} : f64} : (tensor<1x1x{hidden_size}xf32>, tensor<1x1x{hidden_size}xf32>) -> tensor<1x1x{hidden_size}xf32> loc(#loc7)
    ...
    %41 = "top.Tile"(%40) {tile = [1, 1, 1, {kv_tile}, 1]} : (tensor<1x{seq_next}x{num_key_value_heads}x1x{head_dim}xf32>) -> tensor<1x{seq_next}x{num_key_value_heads}x{kv_tile}x{head_dim}xf32> loc(#loc41)
    %42 = "top.Reshape"(%41) {flatten_start_dim = -1 : i64, shape = [1, {seq_next}, {num_attention_heads}, {head_dim}]} : (tensor<1x{seq_next}x{num_key_value_heads}x{kv_tile}x{head_dim}xf32>) -> tensor<1x{seq_next}x{num_attention_heads}x{head_dim}xf32> loc(#loc42)
    ...
    %44 = "top.Tile"(%43) {tile = [1, 1, 1, {kv_tile}, 1]} : (tensor<1x{seq_next}x{num_key_value_heads}x1x{head_dim}xf32>) -> tensor<1x{seq_next}x{num_key_value_heads}x{kv_tile}x{head_dim}xf32> loc(#loc44)
    %45 = "top.Reshape"(%44) {flatten_start_dim = -1 : i64, shape = [1, {seq_next}, {num_attention_heads}, {head_dim}]} : (tensor<1x{seq_next}x{num_key_value_heads}x{kv_tile}x{head_dim}xf32>) -> tensor<1x{seq_next}x{num_attention_heads}x{head_dim}xf32> loc(#loc45)
    ...
    %60 = "top.RMSNorm"(%58, %59) {eps = {rms_norm_eps} : f64} : (tensor<1x1x{hidden_size}xf32>, tensor<1x1x{hidden_size}xf32>) -> tensor<1x1x{hidden_size}xf32> loc(#loc60)
