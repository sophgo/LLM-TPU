...
inline long long
get_elapsed(std::chrono::time_point<
...
void WizardCoder::init(std::string_view model_path, std::string_view vocab_path,
  ...
  [&]() {
    for (int i = 0; i < NUM_LAYERS; i++) {
      ...
      for (int j = 0; j < num_device; j++) {
        ...
        bmrt_tensor_ex(&block.past_cache[j], bmrt,
                       block_net->input_loc_devices[j * in_num + 1],
                       block_net->input_dtypes[j * in_num + 1],
                       block_net->stages[0].input_shapes[j * in_num + 1]);
        ...
        bmrt_tensor_ex(&block.current_cache[j], bmrt,
                       block_net->output_loc_devices[j * out_num + 1],
                       block_net->output_dtypes[j * out_num + 1],
                       block_net->stages[0].output_shapes[j * out_num + 1]);
      }
  ...
  [&]() {
    ...
    bmrt_tensor(&this->lm_head.hidden_states, bmrt, lm_head->input_dtypes[0],
                lm_head->stages[0].input_shapes[0]);
    bmrt_tensor(&this->lm_head.token, bmrt, lm_head->output_dtypes[0],
...
int WizardCoder::forward_first(const std::vector<int> &token_ids) {
  ...
  std::vector<void *> attention_mask_data(num_device,
                                          (void *)attention_mask.get());

  bmrt_memcpy_s2d_parallel(bmrt, &embedding.input_ids_512, tok_id_data.data(),
                           one_input_nums.data(), 1);
  bmrt_memcpy_s2d_parallel(bmrt, &embedding.input_pos_512, pos_id_data.data(),
                           one_input_nums.data(), 1);

  bmrt_memcpy_s2d_parallel(bmrt, blocks.begin()->attention_mask.data(),
                           attention_mask_data.data(),
                           num_device_inputs_nums.data(), num_device);

  bm_tensor_t input_blocks[] = {embedding.input_ids_512,
                                embedding.input_pos_512};

  bmrt_launch_tensor_ex(bmrt, "embedding", input_blocks, 2,
                        &embedding.hidden_states_512, 1, true, false);
  ...
  for (int i = 0; i < NUM_LAYERS; i++) {
    ...
    bmrt_launch_tensor_ex(bmrt, name.c_str(), inputs_block.data(),
                          inputs_block.size(), outputs_block.data(),
                          outputs_block.size(), true, false);
  ...
  auto bytes =
      bm_mem_get_device_size(embedding.hidden_states_512.device_mem) / MAX_LEN;

  bm_memcpy_d2d_byte(handle, lm_head.hidden_states.device_mem, 0,
                     embedding.hidden_states_512.device_mem,
                     (token_length - 1) * bytes, bytes);

  bmrt_launch_tensor_ex(bmrt, "lm_head", &lm_head.hidden_states, 1,
                        &lm_head.token, 1, true, false);
  int token = 0;
...
int WizardCoder::forward_next() {
  ...
  bmrt_memcpy_s2d_parallel(bmrt, &embedding.input_pos_1, input_pid_data.data(),
                           embedding_inputs_num.data(), 1);

  bmrt_tensor_with_device(&embedding.input_ids_1, lm_head.token.device_mem,
  ...
  bmrt_launch_tensor_ex(bmrt, "embedding", input_blocks, 2,
                        &embedding.hidden_states_1, 1, true, false);
  ...
  bmrt_memcpy_s2d_parallel(bmrt, blocks_cache.begin()->attention_mask.data(),
                           attention_mask_data.data(), input_nums.data(),
                           num_device);
  ...
  for (int i = 0; i < NUM_LAYERS; i++) {
    ...
    auto totalsize =
        bm_mem_get_device_size(blocks_cache[0].current_cache[0].device_mem) /
        513;
    for (int j = 0; j < num_device; j++) {
      bm_memcpy_d2d(handle, blocks[i].past_layers[j].device_mem, 0,
                    blocks_cache[i].current_cache[j].device_mem, totalsize,
                    totalsize * 512);
    }
  }

  bmrt_launch_tensor_ex(bmrt, "lm_head", &embedding.hidden_states_1, 1,
                        &lm_head.token, 1, true, false);
  bm_thread_sync(handle);
...
void WizardCoder::stream_generate(const std::vector<int> &input_ids,
                                  int max_new_length) {
  int cnt = 1;
...
void processArguments(int argc, char *argv[], std::string &model_path,
                      std::string &vocab_path, std::vector<int> &devices) {
  ...
  while ((option = getopt_long(argc, argv, "m:d:v:", longOptions,
                               &optionIndex)) != -1) {
    switch (option) {
    case 'm':
      model_path = optarg;
